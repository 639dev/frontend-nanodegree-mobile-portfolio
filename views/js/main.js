...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch (size) {
      case "1":
        document.getElementById("pizzaSize").innerHTML = "Small"; // getElementById will save us time.
        return;
      case "2":
  ...
  function determineDx(elem, size) { //always get elemnts with not an expensive ways like using querySelector 
  ...
  function changePizzaSizes(size) {
    var pizzas = document.getElementsByClassName("randomPizzaContainer"); //storing all pitza elements in an array rather than quering them many times will save us time.
    var dx = determineDx(pizzas[0], size); //taking size of just one pitza will save us much time.
...
function updatePositions() {
  ...
  var scrollTop = document.documentElement.scrollTop; // constant value so why don't we calculate it once?
